...
# remount the contents partition in read-write mode
...
# ext4 partition on the eMMC. Try to guess from /proc/partitions instead of
...
# fallback: look for a system partition in mounts or by name
if [ -z "$ROOTFS_PART" ]; then
    ROOTFS_PART=`cat /proc/mounts | grep -E "system|rootfs" | awk '{ print $1 }' | head -n 1`
fi
